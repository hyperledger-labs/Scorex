...
trait Stubs {
  ...
  val peers = Map(
    inetAddr1 -> PeerInfo(ts1, Some(inetAddr1), version, Some("first"), Some(Incoming), peerFeatures),
    inetAddr2 -> PeerInfo(ts2, Some(inetAddr2), version, Some("second"), Some(Outgoing), peerFeatures)
  )
