...
class TApp(val settingsFilename: String) extends Application {

  override type P = PublicKey25519Proposition
  override type TX = SimpleBoxTransaction
  override type PMOD = HybridBlock
  override type NVHT = TNodeViewHolder
  ...
  override val nodeViewHolderRef: ActorRef = actorSystem.actorOf(Props(classOf[TNodeViewHolder], settings))
  ...
  override val apiTypes: Set[Class[_]] = Set(classOf[UtilsApiRoute], classOf[DebugApiRoute], classOf[WalletApiRoute],
    classOf[NodeViewApiRoute[P, TX]], classOf[PeersApiRoute])
  ...
  override val localInterface: ActorRef = actorSystem.actorOf(Props(classOf[TLocalInterface], nodeViewHolderRef, miner, forger, settings))
