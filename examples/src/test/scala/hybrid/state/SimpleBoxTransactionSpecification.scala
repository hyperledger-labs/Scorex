...
class SimpleBoxTransactionSpecification extends PropSpec
  ...
  property("Transaction boxes are deterministic") {
    ...
    tx.newBoxes.toSeq shouldBe
      Vector(
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3m6nhP4AZjFn5pgMd3PvH6PwHx23AG4tvpLCuu7Wt3hhAPssKc").get.take(33).tail),-6219502975712200872L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("4ZJwiEzpTHhvT6BMYZg1FUXysHkuBLRHb7FvXhZGx6HtsWZCeG").get.take(33).tail),2326174055960855030L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3Y7Ji8wrYZ12EPup6ky2mWEaNo1wTgUKVPJ84xaHwHqTC6LXoh").get.take(33).tail),-2090466149357841238L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3WqPcQ1w1HEaEDvHpnnqqYxJBzQGcf5gT5G5CrsXFL7UX4SA2N").get.take(33).tail),-4786344880748433993L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("4m5cG82kztD9bZVf1Tc1Ni1uvHobpKYuAUyxNSnDm7WLGCZvZh").get.take(33).tail),2879476891976400353L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("4huPANjYcqcdRm99tsCw29JqFnHMTJZsQjoufRQTEDPPoWmPSt").get.take(33).tail),4610029492489107892L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3s3CauhVba81UefEuuaNqRqGLEV9jCZJpvLFg5dJdu29TivRZk").get.take(33).tail),416797087985622128L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3HHuHxBf2eXmbUcGuFCx3dU6Wp7imeRiN5uz4rYDdQwsLwnwW4").get.take(33).tail),-8485818448745401936L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("38uZVfModMnCg5FSECtFiBE7Dbjmh7Tt1SgBD8gFTA1XDHxiqQ").get.take(33).tail),-4750873086163930339L,100000),
        PublicKey25519NoncedBox(PublicKey25519Proposition(Base58.decode("3WTH7tB28nkbC9KFJTy8EBn1bWkxryiLKDnngeP9BYyuCik3aP").get.take(33).tail),1904873933279744536L,100000))
  }
